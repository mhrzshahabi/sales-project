...
function checkContactWithThisProperty(prop_name, prop_value, contact_id,record, warnMsg) {
    isc.RPCManager.sendRequest(
        Object.assign(BaseRPCRequest, {
            actionURL: "${contextPath}/api/contact/spec-list",
            httpMethod: "GET",
            params : {
              criteria: {
            ...
            callback: function (RpcResponse_o) {
                if (RpcResponse_o.httpResponseCode === 200 || RpcResponse_o.httpResponseCode === 201) {
                    let resData = JSON.parse(RpcResponse_o.httpResponseText).response.data;
                    if(resData == null)
                        return;
    ...
    fields: [
        {
            ...
            editorExit(editCompletionEvent, record, newValue, rowNum, colNum, grid) {
                 ...
                 if(newValue != null && newValue.length > 1)
                   checkContactWithThisProperty("phone", newValue,contact_values.id,record,"<spring:message code='contact.phone'/>");
                 return true;
            }
        },
        {
            ...
            editorExit(editCompletionEvent, record, newValue, rowNum, colNum, grid) {
                let contact_values = ValuesManager_Contact.getValues();
                if(newValue != null && newValue.length > 1)
                  checkContactWithThisProperty("mobile", newValue,contact_values.id,record,"<spring:message code='contact.mobile'/>");
                return true;
            }
        },
        {
            ...
            editorExit(editCompletionEvent, record, newValue, rowNum, colNum, grid) {
              let contact_values = ValuesManager_Contact.getValues();
              if(newValue != null && newValue.length > 1)
                checkContactWithThisProperty("fax", newValue,contact_values.id,record,"<spring:message code='contact.fax'/>");
              return true;
            }
        },
        {
