...
public class ForeignInvoiceFormController {
    ...
    public String show(HttpServletRequest request) throws JsonProcessingException {
        ...
        SecurityChecker.addEntityPermissionToRequest(request, ForeignInvoice.class, ForeignInvoicePayment.class, ForeignInvoiceItem.class, ForeignInvoiceItemDetail.class);
    ...
    public void printDocx(HttpServletResponse response, @PathVariable Long foreignInvoiceId) throws IOException {
        ...
        Long materialId = foreignInvoice.getShipment().getMaterial().getId();


        if (materialId == 2) {

            InputStream stream = new ClassPathResource("reports/word/FI_Copper_Cathode.docx").getInputStream();
            XWPFDocument doc = new XWPFDocument(stream);

            Calendar calendar = Calendar.getInstance();
            calendar.setTime(foreignInvoice.getShipment().getSendDate());

            ContractDetailDataDTO.Info contractDetailData = foreignInvoiceService.getContractDetailData(contractId);
            ContractContactDTO.Info seller = foreignInvoice.getShipment().getContractShipment().getContract().getContractContacts().stream().filter(q -> q.getCommercialRole() == CommercialRole.Seller).findFirst().orElseThrow(() -> new NotFoundException(Contact.class));
            ...
            wordUtil.replacePOI(doc, "SELLER_NAME", seller.getContact().getNameEN());
            wordUtil.replacePOI(doc, "SELLER_ADDRESS", seller.getContact().getAddress());
            wordUtil.replacePOI(doc, "SELLER_POSTAL_CODE", seller.getContact().getPostalCode());
            wordUtil.replacePOI(doc, "SELLER_PHONE", seller.getContact().getPhone());
            ...
            wordUtil.replacePOI(doc, "INCOTERM_TITLE", foreignInvoice.getShipment().getIncotermRules().getIncotermRule().getTitleEn());
            ...
            wordUtil.replacePOI(doc, "WEIGHT_GW", String.valueOf(weightGW));
            wordUtil.replacePOI(doc, "WEIGHT_ND", String.valueOf(weightND));

            List<ForeignInvoiceBillOfLandingDTO.Info> billOfLanding = foreignInvoice.getBillLadings();
            billOfLanding.forEach(q -> {
                wordUtil.replacePOI(doc, "BL_NO", q.getBillOfLanding().getDocumentNo());
                wordUtil.replacePOI(doc, "BL_DATE", String.valueOf(q.getBillOfLanding().getDateOfIssue()));
            });

            List<ContractDetailDataDTO.QPData> qpData = contractDetailData.getQP();
            ContractDetailDataDTO.QPData qp = qpData.stream().filter(q -> q.getIncotermRules().getId() == foreignInvoice.getShipment().getIncotermRules().getId()).findFirst().orElseThrow(NotFoundException::new);
            wordUtil.replacePOI(doc, "MOAS_VALUE", String.valueOf(qp.getMoasValue()));
            wordUtil.replacePOI(doc, "SHIPMENT_MONTH", calendar.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.ENGLISH));
            wordUtil.replacePOI(doc, "SHIPMENT_YEAR", String.valueOf(calendar.get(Calendar.YEAR)));

            wordUtil.replacePOI(doc, "UNIT_PRICE", String.valueOf(foreignInvoice.getUnitPrice()));
            wordUtil.replacePOI(doc, "PREMIUM_VALUE", String.valueOf(foreignInvoice.getPremiumValue()));

            wordUtil.replacePOI(doc, "WEIGHT_ND", String.valueOf(foreignInvoice.getPremiumValue()));
            wordUtil.replacePOI(doc, "PREMIUM_VALUE", String.valueOf(foreignInvoice.getPremiumValue()));

            DecimalFormat decimalFormat = new DecimalFormat("#.##");
            BigDecimal finalPrice = weightND.multiply(foreignInvoice.getUnitPrice().add(foreignInvoice.getPremiumValue()));
            wordUtil.replacePOI(doc, "FINAL_UNIT", decimalFormat.format(finalPrice));
            wordUtil.replacePOI(doc, "SUM_PRICE", decimalFormat.format(foreignInvoice.getSumPrice()));

            List<ForeignInvoicePaymentDTO.Info> foreignInvoicePayments = foreignInvoice.getForeignInvoicePayments();
            foreignInvoicePayments.forEach(payment -> {
                BigDecimal shipmentPrice = (payment.getShipmentCostInvoice().getConversionSumPrice().multiply(payment.getPortion())).divide(new BigDecimal(100));
                wordUtil.replacePOI(doc, "PORTION_PERCENT", String.valueOf(payment.getPortion()));
                wordUtil.replacePOI(doc, "SHIPMENT_COST_TYPE", payment.getShipmentCostInvoice().getInvoiceType().getTitle());
                wordUtil.replacePOI(doc, "SHIPMENT_PRICE_CON", decimalFormat.format(shipmentPrice));
            });

            wordUtil.replacePOI(doc, "SUM_CON_PRICE", decimalFormat.format(foreignInvoice.getConversionSumPrice()));
            wordUtil.replacePOI(doc, "CON_TEXT", foreignInvoice.getConversionSumPriceText().toUpperCase());
            wordUtil.replacePOI(doc, "MARKETING_MANAGER", foreignInvoice.getCreator().getFullName());

            response.setHeader("Content-Disposition", "attachment; filename=FI_Copper_Cathode");
            ...
            List<ForeignInvoiceBillOfLandingDTO.Info> billOfLanding = foreignInvoice.getBillLadings();
            billOfLanding.forEach(q -> {
                wordUtil.replacePOI(doc, "SWITCH_DOC_NO", q.getBillOfLanding().getBillOfLadingSwitch() == null ? null : q.getBillOfLanding().getBillOfLadingSwitch().getDocumentNo());
